...
fp = os.path.join(synth_results, f'optimized_results_{new_filename}2.pkl')
with open(fp, 'rb') as handle:
...
df = pd.DataFrame()
df = df.sort_index()
#     idx = pd.date_range(pd.Timestamp('10-01-2019 06:00:00'), pd.Timestamp('10-01-2019 20:55:00'), freq=f"{granularity[i]}min")
...
fp = os.path.join(synth_results, 'Table_2_synth.txt')
with open(fp, 'w') as f:
...
drawn_poly = []
for i, cluster_head in enumerate(list(clusters.keys())[0:]):
...
a = testing_incident_GT_Clist[testing_incident_GT_Clist.index.month == 12].sort_values(by='Total_Number_Incidents', ascending=False)
i = 0
...
# Accuracy is not trust worthy because of the imbalance in false positives and actual incidents
...
# True positive rate (or sensitivity): 𝑇𝑃𝑅=𝑇𝑃/(𝑇𝑃+𝐹𝑁)
# False positive rate: 𝐹𝑃𝑅=𝐹𝑃/(𝐹𝑃+𝑇𝑁)
# True negative rate (or specificity): 𝑇𝑁𝑅=𝑇𝑁/(𝐹𝑃+𝑇𝑁)
...
# Accuracy is not trust worthy because of the imbalance in false positives and actual incidents
# metrics_df['incident_detection_accuracy'] =  (tp + tn) / (tp + tn + fp + fn)
...
# df['average_fa_per_segment'] = df['cluster_false_alm']['mean'] / df['number_of_segmnts']['mean']
...
# ax1.plot(range(1, 10), ith_cluster_df['recall'], mar7er='o', label='Recall')
# ax1.plot(range(1, 10), ith_cluster_df['false_positive_rate'] * 10, marker='o', label='False Alarm Rate')
...
groundtruths = pd.read_pickle(fp)
groundtruths['cluster_head'] = groundtruths['segmentID'].apply(lambda x: locate_segment_cluster(x, clusters))
groundtruths[groundtruths['cluster_head'] == cluster_list[0]]['10-01-2019 06:00:00':'12-31-2019 20:55:00'].between_time('06:00', '20:55').shape
...
#         print(c + k_idx * num_clusters)
        ith_cluster_kth_kappa_df = kappa_df.loc[[c + k_idx * num_clusters]]
#         display(ith_cluster_kth_kappa_df)
        # All metrics including all detection attempts
