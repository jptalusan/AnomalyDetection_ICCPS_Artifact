...
def generate_Baseline_Map():
    ...
    drawn_poly = []
    for _, cluster_head in enumerate(list(clusters.keys())[0:]):
...
def generate_Figure_8a(kappa_df):
    gaps_dict = {}
    ...
    return ith_cluster_df

def generate_Figure_8b(ith_cluster_df):
...
if __name__ == "__main__":
    ...
    print("Training all kappa")
    kappa_df = generate_test_kappa()

    print("Generating figure 8a")
    ith_cluster_df = generate_Figure_8a(kappa_df)

    print("Generating figure 8b")
    generate_Figure_8b(ith_cluster_df)

    time_elapsed = time.time() - time_start
    print(f"Total time for graph generation: {time_elapsed:.4f}")